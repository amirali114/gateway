...
const DEFAULT_STORE_PATH = path.resolve(workspaceRoot, "artifacts/api-server/data/dashboard");
...
function ensureDir(): void {
  fs.mkdirSync(storeDir(), { recursive: true, mode: 0o750 });
}
...
function emptyStore(): StoreFile {
  return { storage_version: 1, users: [] };
}
...
export async function appendAudit({
  ...
  actor?: Pick<DashboardUser, "id" | "username" | "role"> | null;
  action: string;
  target_type: string;
  target_id: string;
  result: AuditResult;
