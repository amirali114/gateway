  ...
  return (
    <>
      ...
      <SectionCard title="Active alerts" description="Duplicate events are grouped by Mother fingerprints. Click Detail to view the full alert record.">
        {activeResult.ok ? active.length ? <DataTable><thead><tr><th>Severity</th><th>Status</th><th>Scope</th><th>Agent</th><th>Title</th><th>First seen</th><th>Last seen</th><th>Count</th><th>Detail</th></tr></thead><tbody>{active.map((a) => <tr key={a.id || a.fingerprint}><td><StatusPill value={a.severity || "info"} /></td><td><StatusPill value={a.status || "active"} /></td><td>{valueOrDash(a.scope)}</td><td className="mono">{valueOrDash(a.agent_id)}</td><td>{valueOrDash(a.title)}</td><td className="mono">{valueOrDash(a.first_seen_at)}</td><td className="mono">{valueOrDash(a.last_seen_at)}</td><td>{valueOrDash(a.occurrence_count)}</td><td>{a.id ? <a href={`/alerts/${encodeURIComponent(a.id)}`}>Detail</a> : "—"}</td></tr>)}</tbody></DataTable> : <EmptyState title="No active alerts" description="Telemetry, storage, and rollout issues will appear here." /> : <ErrorState error={activeResult.error} />}
      </SectionCard>
