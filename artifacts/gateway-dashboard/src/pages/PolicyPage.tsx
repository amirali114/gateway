...
import type { MotherPoliciesResponse, MotherPolicyResponse, ReadyResponse, MotherDiagnosticsSummaryResponse } from "@/lib/types";
...
export default function PolicyPage() {
  ...
  const { data: defaultResult } = useQuery({
  ...
  if (!hasPermission("policy.view")) {
    return <ErrorState title="Access Denied" error="You do not have permission to view policy sync." />;
  }
  ...
  const policySource = ready?.policy_source || (ready as any)?.policy || defaultPolicy?.source;
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Link className="button-link button-secondary" href="/gateway">Gateway control</Link>}
      />
      ...
      <SectionCard title="Policy catalog" description="All policy profiles currently known to Mother.">
        {policiesResult! && policiesResult!.ok ? (
          policies.length ? (
            <DataTable>
              <thead><tr><th>ID</th><th>Profile</th><th>Version</th><th>Source</th><th>Default</th></tr></thead>
              <tbody>
                {policies.map((p) => (
                  <tr key={p.id || p.profile_id}>
                    ...
                    <td><StatusPill value={p.is_default ? "active" : "inactive"} /></td>
                  </tr>
                ))}
              </tbody>
            </DataTable>
          ) : (
            <EmptyState title="No policies returned" />
          )
        ) : (
          <ErrorState error={policiesResult! && !policiesResult!.ok ? (policiesResult as any)!.error : "Unknown error"} />
        )}
      </SectionCard>
