...
import type { MotherAgentsResponse } from "@/lib/types";
...
export default function SyncPage() {
  ...
  if (!hasPermission("agents.view")) {
    return <div className="notice danger">Access denied. You do not have permission to view sync status.</div>;
  }
  ...
  return (
    <>
              ...
              <tbody>
                {agents.map((a, index) => (
                  <tr key={a.agent_id || `agent-sync-${index}`}>
                    <td className="mono">
                      {a.agent_id ? <Link href={`/agents/${encodeURIComponent(a.agent_id)}`}>{a.agent_id}</Link> : "—"}
                    </td>
      ...
      <RawJsonDrawer data={agentsResult} title="Raw sync payload" />
    </>
  );
}
