...
import type { MotherAlertResponse, ReadyResponse, MotherStorageStatusResponse, MotherPoliciesResponse } from "@/lib/types";
...
export default function SettingsPage() {
  ...
  const { data: securityResult } = useQuery({
    ...
    queryFn: () => apiGet<{ auth_enabled: boolean; session_secret_configured: boolean; management_token_configured: boolean; trust_proxy: boolean; user_store_path: string }>("settings/security"),
  });

  const { data: healthResult } = useQuery({
    queryKey: ["mother", "health"],
    queryFn: () => apiGet<MotherAlertResponse>("mother/health"),
  });
  ...
  const { data: policiesResult } = useQuery({
    queryKey: ["mother", "policies"],
    queryFn: () => apiGet<MotherPoliciesResponse>("mother/policies"),
  });

  if (!hasPermission("settings.view")) {
    return <div className="notice danger">Access denied. You do not have permission to view settings.</div>;
  }
  ...
  const motherBaseUrl = "Local-only (server-side)";

  return (
    <>
      <PageHeader eyebrow="Operational" title="Settings & Safety" description="Dashboard configuration and Mother service posture. Actual secrets and tokens are never delivered to the browser." />

      <div className="readonly-banner">
        <span>◈</span>
        <span><b>Read-only configuration visibility.</b> No environment variable, secret, or management token can be modified from this dashboard. All Mother connectivity is read server-side only.</span>
      </div>
      ...
      <div className="grid kpis">
        <KpiCard title="Auth" value={<StatusPill value={security?.auth_enabled ? "enabled" : "disabled"} />} icon="□" />
        <KpiCard title="Mother" value={<StatusPill value={health?.ok ? "healthy" : "unavailable"} />} hint={motherBaseUrl} icon="◎" />
        <KpiCard title="Ready" value={<StatusPill value={ready?.ok ? "ready" : "not-ready"} />} icon="✓" />
        <KpiCard title="Storage" value={<StatusPill value={storage?.engine || "unknown"} />} hint={storage?.writable ? "writable" : "not writable"} icon="▣" />
      </div>

      <div className="grid two" style={{ marginTop: 14 }}>
        <SectionCard title="Dashboard security">
          {security ? (
            <table className="kv"><tbody>
              <tr><th>Auth enabled</th><td><StatusPill value={security.auth_enabled} /></td></tr>
              <tr><th>Session secret</th><td><StatusPill value={security.session_secret_configured ? "configured" : "missing"} /></td></tr>
              <tr><th>Management token</th><td><StatusPill value={security.management_token_configured ? "server-side" : "missing"} /></td></tr>
              <tr><th>Trust proxy</th><td><StatusPill value={security.trust_proxy} /></td></tr>
              <tr><th>User store</th><td className="mono">{valueOrDash(security.user_store_path)}</td></tr>
            </tbody></table>
          ) : (
            <p>Loading security status...</p>
          )}
        </SectionCard>
        <SectionCard title="Mother policies">
          {policies.length ? (
            <DataTable>
              <thead><tr><th>ID</th><th>Profile</th><th>Version</th><th>Source</th></tr></thead>
              <tbody>
                {policies.map((p) => (
                  <tr key={p.id || p.profile_id}>
                    <td className="mono">{valueOrDash(p.id)}</td>
                    <td className="mono">{valueOrDash(p.profile_id)}</td>
                    <td>{valueOrDash(p.version)}</td>
                    <td>{valueOrDash(p.source)}</td>
                  </tr>
                ))}
              </tbody>
            </DataTable>
          ) : (
            <div className="empty-state">No policies returned.</div>
          )}
        </SectionCard>
      </div>
