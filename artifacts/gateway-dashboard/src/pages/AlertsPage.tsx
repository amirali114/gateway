...
import type { MotherAlertSummaryResponse, MotherAlertsResponse } from "@/lib/types";
...
export default function AlertsPage() {
  ...
  const { data: summaryResult } = useQuery({
    ...
    enabled: hasPermission("alerts.view"),
  });

  const { data: activeResult } = useQuery({
    queryKey: ["mother", "alerts", "active"],
    queryFn: () => apiGet<MotherAlertsResponse>("mother/alerts", { status: "active", limit: 200 }),
    enabled: hasPermission("alerts.view"),
  });

  const { data: historyResult } = useQuery({
    queryKey: ["mother", "alerts", "history"],
    queryFn: () => apiGet<MotherAlertsResponse>("mother/alerts", { limit: 100 }),
    enabled: hasPermission("alerts.view"),
  });

  if (!hasPermission("alerts.view")) {
    return <ErrorState title="Access Denied" error="You do not have permission to view alerts." />;
  }

  if (!summaryResult || !activeResult || !historyResult) {
  ...
  return (
    <>
      ...
      <SectionCard title="Active alerts" description="Duplicate events are grouped by Mother fingerprints. Click Detail to view the full alert record.">
        {activeResult.ok ? (
          active.length ? (
            <DataTable>
              <thead><tr><th>Severity</th><th>Status</th><th>Scope</th><th>Agent</th><th>Title</th><th>First seen</th><th>Last seen</th><th>Count</th><th>Detail</th></tr></thead>
              <tbody>
                {active.map((a) => (
                  <tr key={a.id || a.fingerprint}>
                    ...
                    <td className="mono">{valueOrDash(a.first_seen_at)}</td>
                    <td className="mono">{valueOrDash(a.last_seen_at)}</td>
                    <td>{valueOrDash(a.occurrence_count)}</td>
                    <td>{a.id ? <Link href={`/alerts/${encodeURIComponent(a.id)}`}>Detail</Link> : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </DataTable>
          ) : <EmptyState title="No active alerts" description="Telemetry, storage, and rollout issues will appear here." />
        ) : <ErrorState error={activeResult.error} />}
      </SectionCard>
