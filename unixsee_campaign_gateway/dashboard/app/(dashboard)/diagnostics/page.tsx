...
export default async function DiagnosticsPage() {
  ...
  /* R10.16: Alert scope breakdown */
  const byScope = alerts?.by_scope || {};
  const scopeEntries = Object.entries(byScope).sort(([, a], [, b]) => (b as number) - (a as number));

  /* R10.16: Config rollout posture from summary */
  ...
  const rolloutPostureTone: PillTone = configsStale > 0 ? "danger" : configsPending > 0 ? "warning" : configsAcknowledged > 0 ? "success" : "neutral";
  ...
  return (
    <>
      ...
      {/* R10.16: Alert posture — severity breakdown with scope grouping */}
      <SectionCard title="Alert posture" description="Active alert counts by severity and scope. Resolved counts cover the last 24 hours.">
        ...
        {/* R10.16: Alert scope breakdown table */}
